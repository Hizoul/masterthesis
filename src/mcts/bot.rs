...
impl MCTSBot {
  ...
  fn simulate_tree(field: &mut GameField, root: &ThreadSafeNode, player: u8, exploration_constant: f64, use_rave: bool, rave_beta_param: f64, transposition_table: Arc<RwLock<HashMap<Vec<u8>, ThreadSafeNode>>>) -> ThreadSafeNode {
    ...
    while !field.game_over {
      ...
      field.place_block_using_play(action as u8);
      ...
      if is_parent {
        ...
        if depth >= 4 {
          let found_child = writeable_table.get(&rendered_field);
        ...
        if existing_child.is_some() {
          ...
          if depth >= 4 {
            let write_res = writeable_table.insert(rendered_field, new_child.clone());
  ...
  fn select_move(node: &ThreadSafeNode, field: &GameField, player: u8, exploration_constant: f64, use_rave: bool, rave_beta_param: f64) -> usize {
      ...
      if use_rave && !field.game_over {
        ...
        if is_parent {
          ...
          if existing_child.is_some() {
            ...
            let new_child = Node::create_child(&current_leaf, action as usize, field.possible_plays.len());
            if depth >= 4 {
              let write_res = writeable_table.insert(rendered_field, new_child.clone());
              if write_res.is_some() {
                println!("CONFLICT IN TRANSPOISTION TABLE DETECTED!");
...
impl Bot for MCTSBot {
  ...
  fn get_name(&self) -> String {format!("mcts_{}_{}_{}", self.use_rave, self.use_pool_rave, self.thought_time)}
}